...
| デプロイ方式 | AWS CDK または Terraform（未定） |
...
| サブネット名 | CIDR | 種別 | 用途 |
|---|---|---|---|
| public-1a | 10.0.1.0/24 | パブリック | ALB |
| public-1c | 10.0.2.0/24 | パブリック | ALB |
| private-1a | 10.0.11.0/24 | プライベート | ECS、RDS、ElastiCache |
| private-1c | 10.0.12.0/24 | プライベート | ECS、RDS、ElastiCache |
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      "Effect": "Allow",
      "Action": "iot:Publish",
      "Resource": "arn:aws:iot:ap-northeast-1:*:topic/home/devices/${iot:Connection.Thing.ThingName}/data"
    }
  ]
}
```
...
| エラーアクション | なし（転送失敗はIoT Core組み込みリトライに委ねる） |
...
- RDS INSERT + 閾値チェック + SNS Publish の最大処理時間 ≒ 10秒
- 指数バックオフリトライ（最大60秒）を考慮し 30秒に設定
...
| 項目 | 値 |
|---|---|
| メンテナンスウィンドウ | 月曜 15:00〜16:00 UTC |
...
2. ECS RunTask API を呼び出しバッチタスクを1回実行
...
| RDS 障害 | CloudWatch（RDS標準メトリクス）→ メール通知 | マルチAZ構成のため自動フェイルオーバー（数十秒）。復旧後にDLQリドライブ実施 |
| ECS Worker 停止 | ECS サービスが自動タスク再起動 | 再起動後は SQS VisibilityTimeout 経過後にメッセージを再受信 |
