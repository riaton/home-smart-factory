...
> **設計メモ:** anomaly_logs の INSERT に失敗した場合、SNS 通知も行わない。通知の根拠となるログが存在しない状態での通知を防ぐため。異常検知の失敗は CloudWatch Logs で検知する。
...
sequenceDiagram
    ...
    alt SNS 一時障害
        ...
        Note over Worker: 指数バックオフでリトライ<br/>（1s → 2s → 4s → 最大60s）
...
| ECS(Worker) → anomaly_logs | RDS一時障害 | エラーログ出力・SNS通知しない | あり（異常ログ） |
