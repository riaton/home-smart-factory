...
sequenceDiagram
    ...
    API->>Logs: SELECT WHERE user_id = ?<br/>ORDER BY detected_at DESC<br/>LIMIT per_page OFFSET (page-1)*per_page
...
sequenceDiagram
    ...
    API->>Logs: SELECT WHERE user_id = ?<br/>AND device_id = ? AND metric_type = ?<br/>AND detected_at BETWEEN ? AND ?<br/>ORDER BY detected_at DESC<br/>LIMIT per_page OFFSET (page-1)*per_page
    Logs-->>API: 絞り込み結果 + 総件数
