...
sequenceDiagram
    ...
    API->>API: バリデーション<br/>・metric_type が temperature / humidity / power_w のいずれか<br/>・power_w の場合 min_value は無視<br/>・min_value と max_value を両方省略していないか

    API->>Thresholds: INSERT INTO anomaly_thresholds<br/>(user_id, metric_type, min_value, max_value, enabled)
    Thresholds-->>API: 成功
