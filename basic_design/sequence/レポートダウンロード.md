...
sequenceDiagram
    ...
    API->>Downloads: SELECT COUNT(*)<br/>WHERE user_id = ? AND download_date = 当日(JST)
    Downloads-->>API: download_count_today

    API-->>React: 200 OK<br/>{ data: { ..., download_count_today: 1 } }
    Note over React: download_count_today >= 3 の場合<br/>ダウンロードボタンを非活性にする
```
...
sequenceDiagram
    ...
    API->>API: レポートデータからPDF生成

    API->>Downloads: INSERT INTO report_downloads<br/>(user_id, report_id, download_date, downloaded_at)
    Downloads-->>API: 成功

    API-->>React: "200 OK<br/>Content-Type: application/pdf<br/>Content-Disposition: 
```
