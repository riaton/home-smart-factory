...
sequenceDiagram
    ...
    alt RDS 一時障害
        ...
        Note over Worker: SNS 通知は行わない<br/>（ログ未保存のまま通知しない）
        Note over Worker: エラーログを出力して終了<br/>（SQSメッセージは DeleteMessage 済みのため再処理なし）
    end
```

> **設計メモ:** anomaly_logs の INSERT に失敗した場合、SNS 通知も行わない。通知の根拠となるログが存在しない状態での通知を防ぐため。異常検知の失敗は CloudWatch Logs Insights で「ERROR」「anomaly_logs INSERT failed」を検索して検知する。
...
sequenceDiagram
    ...
    alt SNS 一時障害
        ...
        Note over Worker: エラーログを出力して終了<br/>通知の失敗は許容する<br/>（anomaly_logs は保存済みのためデータロストなし）
    else 宛先不正
        SNS-->>Worker: 400 InvalidParameter
        Note over Worker: リトライしない<br/>エラーログを出力して終了
    end
```

> **設計メモ:** SNS 通知失敗は許容する。anomaly_logs への保存が完了していれば、ユーザーは画面から異常一覧を確認できる。通知はベストエフォートとする。
...
| ECS(Worker) → anomaly_logs | RDS一時障害 | エラーログ出力・SNS通知しない | あり（異常ログ） CloudWatch Logs Insightsで「ERROR」「anomaly_logs INSERT failed」を検索 |
| ECS(Worker) → SNS | SNS一時障害 | 指数バックオフでリトライ | なし（ログは保存済み） |
| ECS(Worker) → SNS | リトライ上限超過 | エラーログ出力・通知失敗を許容 | なし（ログは保存済み） |
| ECS(Worker) → SNS | 宛先不正（400） | リトライしない・エラーログ出力 | なし（ログは保存済み） |
